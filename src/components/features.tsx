...
export const Features = () => {
  return (
    <section id="features" className="py-20 px-6 md:px-10 lg:px-20 relative theme-transition overflow-hidden">
      {/* Gradient background overlay */}
      <div className="absolute inset-0 bg-secondary/30 dark:bg-secondary/10"></div>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <div 
              key={index} 
              className={`bg-card p-6 rounded-lg shadow-md hover:shadow-lg transition-all duration-300 border border-border reveal hover-lift hover-card ${index % 3 === 0 ? 'reveal-delay-1' : index % 3 === 1 ? 'reveal-delay-2' : 'reveal-delay-3'} relative overflow-hidden cursor-pointer`}
            >
              {/* Subtle gradient background */}
              <div className="absolute inset-0 gradient-bg-secondary opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-lg"></div>
              ...
              <div className="mb-4 bg-brand-light-purple/50 dark:bg-brand-purple/20 p-3 inline-block rounded-lg animate-pulse-slow relative z-10">
                {feature.icon}
              </div>
              <h3 className="text-xl font-bold mb-2 text-foreground relative z-10">{feature.title}</h3>
              <p className="text-foreground/80 relative z-10">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
